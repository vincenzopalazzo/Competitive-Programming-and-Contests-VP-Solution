...
class PersistentSegmentTree
{
	    ...
	    std::shared_ptr<Node<T>> build_structure(int left_index, int right_index)
      {
	    ...
	    std::shared_ptr<Node<T>> build_structure_procedure(int left_index,  int right_index)
	    {
          ...
          return std::make_shared<Node<T>>(1);
		    }
	...
	int range_query_subroutine(std::shared_ptr<Node<T>> &node,
				   int left_index, int right_index,
				   int query_left, int query_right)
	{
		// outside the range
		if (query_left > query_right || !node)
			return -1;
		// range represented by a node is completely inside the given
		// range
		...
		int left_segment = range_query_subroutine(
		    node->left, left_index, middle_point, query_left,
		    std::min(query_right, middle_point));
		int right_segment = range_query_subroutine(
		    node->right, middle_point + 1, right_index,
		    std::max(query_left, middle_point + 1), query_right);
		if (left_segment == -1)
			return right_segment;
		if (right_segment == -1)
			return left_segment;
		return (left_segment <= right_segment) ? left_segment
						       : right_segment;
	}

	std::shared_ptr<Node<T>>
	update_range_subroutine(std::shared_ptr<Node<T>> &node, int left_index,
				int right_index, int pos, T new_val)
	{
		...
		int middle_point = (left_index + right_index) / 2;
		if (pos <= middle_point) {
			auto left_node = update_range_subroutine(
			    node->left, left_index, middle_point, pos, new_val);
			return std::make_shared<Node<T>>(left_node,
							 node->right);
		}
		auto right_node = update_range_subroutine(
		    node->right, middle_point + 1, right_index, pos, new_val);
	...
	{
		return range_query_subroutine(root, 0, origin.size(),
	...
	{
		auto new_root =
		    update_range_subroutine(root, 0, origin.size(), at, value);
