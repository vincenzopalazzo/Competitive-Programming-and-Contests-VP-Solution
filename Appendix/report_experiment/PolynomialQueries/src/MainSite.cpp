...
class LazySegmentTree {
 protected:
   ...
   * the logic inside the segment tree this version store inside the node of
   * Segment Tree the value and not the index of the value
   * @tparam T is the type of structure, usually it is a int or another numeric
   * type
   * @param start_index
   * @param left_index
  ...
  T range_query_subroutine(std::size_t start_index, std::size_t left_index,
    ...
    auto right_child = right_child_index(start_index);
    auto left_segment =
        range_query_subroutine(left_child, left_index, middle_point, query_left,
                               std::min(middle_point, query_right));
   ...
   * @param left_index: At the starting point it is the first position of the
   * original array, so 0.
   * @param right_index: At the starting point it is the last position of the
   * original array, so size - 1
  ...
  void update_range_subroutine(std::size_t start_index, std::size_t left_index,
    ...
    if ((left_index >= from) && (right_index <= to)) {
      ...
      auto left_subtree = (lazy[left_child] != -1)
                          ? (lazy[left_child] + structure[left_child])
                          : structure[left_child];
      auto right_subtree = (lazy[right_child] != -1)
                           ? (lazy[right_child] + structure[right_child])
  ...
  std::size_t left_child_index(const std::size_t index) { return index * 2; }
 ...
 public:
  ...
  /**
   * This procedure update the value stored inside the structure in one single
   * position, this use the update_range_subroutine with from == to == at; to
   * run the procedure update
...
struct Query {
  ...
  std::size_t start;
  std::size_t end;
  Query(uint type, size_t start, size_t anEnd)
      : type(type), start(start - 1), end(anEnd - 1) {}
};

template <typename T, typename R>
static std::vector<R> exponential_sum_in_a_range(std::vector<T> &inputs,
                                                 std::vector<Query> const &queries) {
  std::vector<R> results;
  results.reserve(queries.size());  // over estimation
  auto segment_tree = cpstl::LazySegmentTree<T>(inputs);
  for (auto &query : queries) {
    if (query.type == 1) {
      T counter = 1;
      for (std::size_t i = query.start; i <= query.end; i++) {
        segment_tree.update(i, counter);
        counter++;
      }
    } else {
...
int main() {
  ...
  for (std::size_t t = 0; t < Q; t++) {
    ...
    cin >> start;
    cin >> end;
    queries.emplace_back(type, start, end);
  }

  auto results =
      exponential_sum_in_a_range<long long, long long>(inputs, queries);
