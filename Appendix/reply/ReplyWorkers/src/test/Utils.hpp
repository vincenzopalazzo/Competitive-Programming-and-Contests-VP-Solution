...
namespace cpstl {
  ...
  std::vector<std::string> tokenize_input_into_strings(std::string const &input_path)
  {
  ...
  std::vector<std::string> tokenize_string_by_simbol(std::string &line, char symbol)
  {
    ...
    for (auto elem : line) {
      if (elem == symbol) {
        ...
        cp_log(INTERNAL_LOG, "Push token: " + token);
        token = "";
      } else {
        token += elem;
      }
