...
#include <algorithm>
#include <vector>
#include "DataStructureFrogAndMosquitos.hpp"
...
class LazySegmentTree {
 protected:
  ...
  void update_range_subroutine(std::size_t start_index, std::size_t left_index,
    ...
    if ((left_index >= from) && (right_index <= to)) {
      ...
      structure[start_index] = std::max(left_subtree, right_subtree);
    }
  }
 ...
 public:
  LazySegmentTree(std::vector<Frog> &origin) : origin(origin) {
    std::size_t size = origin.size();
    structure = std::vector<T>(size * 4);
    lazy = std::vector<T>(size * 4, 0);
    origin = origin;
