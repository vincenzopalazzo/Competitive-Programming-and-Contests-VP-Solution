...
static void
increment(std::vector<Tree<T>> const &input, std::vector<T> &counter, std::vector<R> &answer, std::size_t position) {
...
template<typename T, typename R>
static void
decrement(std::vector<Tree<T>> const &input, std::vector<T> &counter, std::vector<R> &answer, std::size_t position) {
    auto to_pos = input[position].key;
    auto count = counter[to_pos];
    answer[count]--;
    counter[to_pos]--;
...
int main() {
    ...
    for (std::size_t q = 0; q < Q; q++) {
        int vertex, kth_color;
        scanf("%d%d", &vertex, &kth_color);
        queries.emplace_back(vertex, kth_color, inputs.size(), q);
    }
    auto results = numbers_of_vertices_with_color<int, int>(inputs, edges, queries);
