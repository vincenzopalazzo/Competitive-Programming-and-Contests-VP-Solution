...
{
	std::string::iterator new_end = unique(
	...
	splits.push_back(input_string.substr(
	    i, std::min(pos, input_string.length()) - i + 1));
...
template <typename T>
inline void cp_log(Log log, std::vector<std::vector<T>> const &inputs)
{
	if (!log.enable)
		return;
	std::cout << BOLDYELLOW;
