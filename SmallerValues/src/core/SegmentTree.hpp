...
class SegmentTree {
 private:
  ...
  std::vector<T> range_query_subroutine(int start_index, int left_index_now,
  ...
  void update_subroutine(int start_index, int left_index, int right_index,
    ...
    if (left_index == right_index) {
      ...
      auto segment_left = structure[left_child];
      auto segment_right = structure[right_child];
      structure[start_index] = segment_left.insert(
          segment_left.end(), segment_right.begin(), segment_right.end());
    }
  }

  inline int left_child_index(const int index) { return index * 2; }

  inline int right_child_index(const int index) { return (index * 2) + 1; }
