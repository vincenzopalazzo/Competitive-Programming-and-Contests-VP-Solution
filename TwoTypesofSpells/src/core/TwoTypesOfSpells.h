...
R make_final_sum(DoubledCounter<T> &doubled_counter, SpellsByType<T> &spells, TotSum<R> &tot_sum,
                 std::size_t spells_doubled) {
    auto sum_double = doubled_counter.lightning + doubled_counter.lightning;
    while (spells.lightning.size() < sum_double)
        update_status_by_spell(spells, tot_sum, doubled_counter, spells_doubled, true);
    while (spells.lightning.size() < sum_double)
        update_status_by_spell(spells, tot_sum, doubled_counter, spells_doubled, false);
